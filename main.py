...
def generate_large_synthetic_data_with_smaller_chunks(df, start_date, num_chunks, chunk_size):
    ...
    for i in range(num_chunks):
        ...
        chunk_start_date = pd.to_datetime(start_date) + pd.DateOffset(days=i * chunk_size)
        date_range = pd.date_range(start=chunk_start_date, periods=chunk_size, freq='D')
        ...
        chunk_data['Leave_date'] = pd.to_datetime(chunk_data['Leave_date'], format='%m/%d/%Y')
        chunk_data['Return_date'] = pd.to_datetime(chunk_data['Return_date'], format='%m/%d/%Y')
        ...
        synthetic_data = pd.concat([synthetic_data, chunk_data], ignore_index=True)
    
    # Ensure sufficient anomalies are included
    anomaly_count = len(synthetic_data) // 100  # Assume 1% of the rows should be anomalies
...
num_chunks = 100
chunk_size = 1000
large_synthetic_data = generate_large_synthetic_data_with_smaller_chunks(original_data, '2024-01-01', num_chunks, chunk_size)

# Save to CSV
large_synthetic_data.to_csv('/mnt/data/large_synthetic_aania_schedule.csv', index=False)
